...
(function () {
  ...
  Brush.prototype = {
    ...
    drawFromLast: function drawFromLast() {
      var v = this._getV();
      ...
      if (spacing > 0) {
        ...
        while (timeout > 0 && (v > lastV && curV <= v || v < lastV && curV >= v)) {
          ...
          this._render(curV, {
            rotation: 0,
            pressure: Math.sqrt(1 - Math.abs(v - .5) * 2)
          });
    ...
    _vToLen: function _vToLen(v) {
      return v * this.pathLength;
    },
    _lenToV: function _lenToV(len) {
      return len / this.pathLength;
    },
    _getV: function _getV() {
      return this._cleanV(this.v);
    },
    _cleanV: function _cleanV(v) {
      v = checkNumber(v, this.v);
      if (v > 1) v = 1;
      if (v < 0) v = 0;
      return v;
    },
    _render: function _render(v, options) {
      v = this._cleanV(v);
      options = checkObject(options);
  ...
  BrushTip = function BrushTip(options) {
    options = checkObject(options);
  ...
  BrushTip.prototype = {
    ...
    _currentCtx: null,
    _currentOptions: {},
    draw: function draw(ctx, options) {
      options = checkObject(options);
    ...
    _drawCircle: function _drawCircle(r, x, y, polar) {
      x = checkNumber(x, 0);
      y = checkNumber(y, 0);
